...
def gc_content(seq):
    ...
    seq = str(seq).upper()
    return round((seq.count("G") + seq.count("C")) / len(seq) * 100, 2) if len(seq) > 0 else 0
...
def run_analysis(fna_path, gbff_path=None, faa_path=None, results_dir="results"):
    ...
    seq_data = [
        {"id": rec.id, "gc_percent": gc_content(rec.seq)}
        for rec in seq_records
    ]
    df_seq = pd.DataFrame(seq_data)
    fig1, ax1 = plt.subplots(figsize=(10, 6))
    sns.barplot(data=df_seq, x="id", y="gc_percent", hue="id", palette="viridis", ax=ax1)
    ...
    if gbff_path:
        ...
        fig2.tight_layout()
        df_genes.to_csv(os.path.join(results_dir, "gc_content_per_gene.csv"), index=False)
    ...
    if faa_path:
        ...
        sns.barplot(x="AminoAcid", y="Frequency", data=df_aa, hue="AminoAcid", palette="mako", ax=ax3)
        ax3.set_ylabel("Relative Frequency")
        ax3.set_xlabel("Amino Acid")
        ax3.set_title("Amino Acid Composition")
        fig3.tight_layout()
        df_aa.to_csv(os.path.join(results_dir, "amino_acid_composition.csv"), index=False)
